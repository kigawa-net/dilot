...
**動作**
1. コンフィグファイルのパスを解決する（優先順位は `new` コマンドと同様）
2. コンフィグファイルが存在しない場合は空一覧として扱い（0件）正常終了する
3. コンフィグファイルが存在する場合は読み込む
4. `projects` 配列の各エントリを1行ずつ標準出力に出力する

**出力フォーマット**

```
<name>\t<coreUrl>\t<createdAt>
```

- フィールドはタブ区切り
- プロジェクトが0件の場合は何も出力しない（終了コード0）

**出力例**

```
my-project    https://github.com/example/repo.git    2026-05-14T00:00:00Z
other-proj    git@github.com:user/other.git           2026-05-20T12:00:00Z
```
...
## コミュニケーション規約設計（refs #34）

### 概要

このリポジトリに関する会話・進捗報告・レビュー・issue/PRコメントを原則として日本語で行う規約を `docs/dev.md` と `CLAUDE.md` に追加する。

### 変更ファイル一覧（#34）

| ファイル | 変更種別 | 内容 |
|---|---|---|
| `docs/dev.md` | 変更 | コミュニケーション規約セクションを追加 |
| `CLAUDE.md` | 変更 | エージェント向けの日本語会話ルールを追加 |
...
実装完了PRのマージ時にGitHubの自動クローズ機能でissueも同時に閉じる規約を `docs/dev.md` および `CLAUDE.md` に明記する。

### 変更方針

現在の `docs/dev.md` では PR本文に `Closes #<番号>` または `refs #<番号>` のどちらでもよいと記述されている。
これを以下のように区別する:

| 状況 | 使用するキーワード |
|---|---|
| 実装完了PR（issueを閉じる） | `Closes #<番号>` |
| 計画PR・作業途中PR（issueを閉じない） | `refs #<番号>` |

GitHubの自動クローズキーワード（`Closes`/`Fixes`/`Resolves`）を使うことで、PRマージと同時にissueがクローズされる。

### 変更ファイル一覧（#37）

| ファイル | 変更種別 | 内容 |
|---|---|---|
| `docs/dev.md` | 変更 | PRの規約セクションにキーワード使い分けルールを追記 |
| `CLAUDE.md` | 変更 | エージェント向けに同ルールを追記 |
